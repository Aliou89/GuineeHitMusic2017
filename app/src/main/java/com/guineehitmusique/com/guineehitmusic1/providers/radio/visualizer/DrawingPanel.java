package com.guineehitmusique.com.guineehitmusic1.providers.radio.visualizer;
...
public class DrawingPanel extends SurfaceView implements SurfaceHolder.Callback {
	...
	public DrawingPanel(Context context, int vType, int cMode, int cMode2, boolean freq, int audioSessionID) {
		super(context);
		frequency = freq;
		displayType = vType;
		cToggle = cMode;
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		if (grabAudio == null){
			if (frequency){
				grabAudio = new GrabAudio(1, resolution, audioSessionID);
			}
			else {
				grabAudio = new GrabAudio(0, resolution, audioSessionID);
			}
		}
		grabAudio.start();
		getData();
