package com.guineehitmusique.com.guineehitmusic1.providers.wordpress;
...
public class WordpressListAdapter extends ArrayAdapter<PostItem> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (position == 0 && (null != newsItem.getAttachmentUrl() && !newsItem.getAttachmentUrl().equals("")) && !simpleMode){
            ...
            Picasso.with(mContext).load(newsItem.getAttachmentUrl()).placeholder(R.drawable.placeholder).into((ImageView) convertView.findViewById(R.id.imageViewHighlight));
		...
		if (null == newsItem.getThumbnailUrl() || newsItem.getThumbnailUrl().equals("") || newsItem.getThumbnailUrl().equals("null")){
			...
			holder.imageView.setVisibility(View.VISIBLE);
            Picasso.with(mContext).load(newsItem.getThumbnailUrl()).fit().centerInside().into(holder.imageView);
        }
